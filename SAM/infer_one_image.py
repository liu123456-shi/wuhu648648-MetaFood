...
if __name__ == "__main__":
    ...
    **期望的输出格式 (示例):**
    ...
    text2 = ""
    # 图片路径
    local_image_path = "./food_1/food_combos/4-cheesecake_strawberry_raspberry/4.JPG"
    # 文件夹名称
    name = "4-cheesecake_strawberry_raspberry"
    ...
    # [之前的 Python 代码，这里省略以保持简洁]
    # ... (导入库，定义 system_prompt, local_image_path, name 等) ...
    # ... (读取图片、编码、准备 API 请求) ...
    ...
    try:
        ...
        # --- 开始：提取并解析 JSON ---
        if completion.choices:  # 检查 API 是否成功返回了 choices 列表
            message_content = completion.choices[0].message.content  # 获取模型返回的消息内容
            text2 = message_content
            print(f"原始响应内容:\n{message_content}")  # 打印原始响应，用于调试

    except Exception as e:
        # 捕获调用 API 时可能发生的网络或其他错误
        print(f"调用API时出错: {e}")
    ...
    try:
        ...
        if completion.choices:  # 检查 API 是否成功返回了 choices 列表
            ...
            # 清理字符串：移除 markdown 标记和首尾空白
            cleaned_content = message_content.strip()  # 移除首尾可能存在的空白字符
            if cleaned_content.startswith("```json"):
                # 如果以 "```json" 开头，移除这部分并再次移除可能出现的空白
                cleaned_content = cleaned_content[len("```json"):].strip()
            elif cleaned_content.startswith("```"):  # 处理可能只有 "```" 的情况
                cleaned_content = cleaned_content[len("```"):].strip()

            if cleaned_content.endswith("```"):
                # 如果以 "```" 结尾，移除这部分并再次移除可能出现的空白
                cleaned_content = cleaned_content[:-len("```")].strip()

            print(f"\n清理后的内容:\n{cleaned_content}")  # 打印清理后的内容，用于调试
            ...
            # 如果 API 响应中没有 'choices'
            print("错误：API响应中未找到任何 'choices'。")
            print("完整响应:", completion)  # 打印完整的响应对象以便排查问题
